    ...
    for llm_model_name in filenames[::-1]:
        ...
        for i in trange(0, len(dataset), args.batch_size):
            ...
            with torch.no_grad():
                ...
                print(f"Model output keys: {llm_output.keys()}")
                assert "hidden_states" in llm_output, f"{llm_model_name} didn't return hidden states"
                assert llm_output["hidden_states"][0].dim() == 3, f"{llm_model_name} hidden state dim mismatch"

                if torch.isnan(loss).any():
                    print(f"[Warning] NaN loss encountered for {llm_model_name}. Skipping batch.")
                    continue
